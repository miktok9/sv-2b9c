...
def main():
    ...
    if story_file.exists():
        ...
        title = title_parts[0][:100] if title_parts else "Kvinnornas historia i antiken"
    else:
        title = f"Kvinnornas historia i antiken - {datetime.date.today()}"

    # Platform-specific content
    descriptions = {
        'youtube': f"{story[:150] if len(story) > 150 else story} #Kvinnohistoria #AntikHistoria #Historia #Utbildning",
        'instagram': f"{story[:2200] if len(story) > 2200 else story}\n\n#Kvinnohistoria #AntikHistoria #Historia #Utbildning #Shorts #Reels",
        'tiktok': f"{story[:2200] if len(story) > 2200 else story} #Kvinnohistoria #AntikHistoria #Historia #Utbildning #FYP",
        'facebook': f"{story[:63206] if len(story) > 63206 else story}\n\n#Kvinnohistoria #AntikHistoria #Historia #Utbildning",
        'threads': f"{story[:500] if len(story) > 500 else story} #Kvinnohistoria #AntikHistoria #Historia #Utbildning",
        'twitter': f"{story[:280] if len(story) > 280 else story} #Kvinnohistoria #AntikHistoria #Historia",
        'vk': f"{story[:220] if len(story) > 220 else story}\n\n#Kvinnohistoria #AntikHistoria #Historia #Utbildning",
        'telegram': f"{story[:4096] if len(story) > 4096 else story}\n\n#Kvinnohistoria #AntikHistoria #Historia #Utbildning"
    }

    tags = [
        'Historia', 'Antika kvinnor', 'Historiska fakta',
        'Shorts', 'Reels', 'AI', 'Utbildning'
    ]
    ...
    if all([
        ...
        print("="*60)
        try:
            result = upload_to_telegram(str(video_file), descriptions['telegram'])
            results['telegram'] = result
            print(f"✅ Telegram: Message ID {result.get('result', {}).get('message_id', 'unknown')}")
        except Exception as e:
